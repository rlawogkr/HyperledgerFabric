...
const aaas = ['Tomoko', 'Brad', 'Jin Soo', 'Max', 'Adrianna', 'Michel', 'Aarav', 'Pari', 'Valeria', 'Shotaro'];
const bbbs = ['Tomoko', 'Brad', 'Jin Soo', 'Max', 'Adrianna', 'Michel', 'Aarav', 'Pari', 'Valeria', 'Shotaro'];
...
class CreateCarWorkload extends WorkloadModuleBase {
    ...
    async submitTransaction() {
        this.txIndex++;
        let carNumber = 'Client' + this.workerIndex + '_FIRMWARE' + this.txIndex.toString();
        ...
        let aaa = aaas[Math.floor(Math.random() * aaas.length)];
        let bbb = bbbs[Math.floor(Math.random() * bbbs.length)];


        let args = {
            contractId: 'firmware',
            contractVersion: 'v1',
            contractFunction: 'createFirmware',
            contractArguments: [carNumber, carMake, carModel, carColor, carOwner,aaa,bbb],
            timeout: 30
        };
